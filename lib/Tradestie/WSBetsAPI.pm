...
sub date_formatter($self, $month = '11', $day = '17', $year = '2022'){
    croak "Error: The month given is not a valid integer or date" unless int($month);
    
    croak "Error: The day given is not a valid integer or date" unless int($day); 
    
    croak "Error: The year given is not a valid integer or date" unless int($year);

    croak "Error: The month must contain a two digit value" unless length($month) == 2;

    croak "Error: The day must contain a two digit value" unless length($day) == 2;

    croak "Error: The year must contain a four digit value" unless length($year) == 4;
...
sub reddit( $self, $date = undef ) {
    ...
    for my $response (@$response_list) {
        push @responses,
          Tradestie::WSBetsAPI::Reddit->new($response);
    }
